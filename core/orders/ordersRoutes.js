...
const JewelryOrder = require("./ordersSchema");
const {
  appendToSheet,
  updateSheetRow,
} = require("../../utils/googleSheets.service");
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // ✅ TRIGGER GOOGLE SHEET SYNC HERE
      //appeding NewOrder to google sheet
      await appendToSheet(savedOrder);

      res.status(201).json({
        success: true,
        message: "Order created successfully and synced to Sheet",
        data: savedOrder,
      });
    } catch (error) {
...
router.delete("/delete/:id", verifyToken, async (req, res) => {
  try {
    ...
    const deletedOrder = await JewelryOrder.findByIdAndDelete(orderId);
    if (!deletedOrder)
      return res.status(404).json({ message: "Order not found" });
