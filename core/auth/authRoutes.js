...
const User = require("./authSchema"); // Adjust the path as needed
const authorizeRoles = require("../../middleware/checkRoles"); // Adjust the path as needed
...
router.get("/users", verifyToken, async (req, res) => {
  try {
    // Destructure role and status from query params
    const { username, role, status } = req.query;
    let query = {};

    // 1. Username filter (Search "contains")
    if (username) {
      query.username = { $regex: username, $options: "i" };
    }

    // 2. Role filter (Exact match)
    if (role) {
      query.role = role;
    }
    ...
    // Search with dynamic query object
