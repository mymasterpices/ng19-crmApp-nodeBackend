...
router.post('/search', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    console.log(`📷 ${req.file.originalname} | ${req.file.mimetype} | ${req.file.size} bytes`);
    ...
    if (data.warning) {
      return res.json({ success: false, message: data.warning, count: 0, results: [] });
    }
    ...
    res.status(500).json({ success: false, message: 'Search failed. Check Python service.' });
  }
});
